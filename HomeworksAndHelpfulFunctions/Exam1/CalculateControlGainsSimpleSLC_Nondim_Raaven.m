function [control_gains, linear_terms]=CalculateControlGainsSimpleSLC_Nondim_Raaven(aircraft_parameters, trim_definition, trim_variables)
%
...
%%%%%%%
% course hold gains
W_X = 15;    %bandwidth separation from roll to course hold SELECT
wn_chi = (1/W_X)*wn_roll; 
zeta_chi = 1.5; %%%<---------- STUDENT SELECT
...
control_gains.Kd_beta = (1/a_beta2)*((a_beta1 + a_beta2*control_gains.Kp_beta)/(2*zeta_beta))^2;
control_gains.Ki_beta = 0.1; %%%<---------- STUDENT SELECT
...
W_h = 20;    %Bandwidth separation from pitch to height SELECT
wn_height = (1/W_h)*wn_pitch;
...
W_v2 = 1;   %Bandwidth separation from pitch to airspeed SELECT
wn_airspeed = (1/W_v2)*wn_pitch; 
...
W_t = 40;    %Throttle Bandwidth Separation SELECT
wn_airspeed = (1/W_t)*wn_pitch;
zeta_airspeed = 1; %%%<---------- STUDENT SELECT
